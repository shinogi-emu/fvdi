...
/*
 * id = lib_vqt_name_and_id(format, name_in, count, name_out)
 *
 * NVDI's VDI 230 sub 100: given a font name, return the ID a program can
 * hand to vst_font(), plus the name the driver actually matched.
 *
 * It is what turns an enumerated font into a usable one.  A program walks
 * the faces with vqt_xfntinfo(), which reports names, and then has to get
 * back from a name to an ID -- so without this a font list can be complete
 * and still entirely unusable, which is exactly the state fVDI was in.
 *
 * The caller passes one character per intin word (gemlib's
 * vdi_str2array_n), not a packed string, and `count` is how many of them
 * there are -- the array is not required to be terminated.
 *
 * `format` is accepted and ignored.  NVDI uses it to select which of a
 * font's several names is being offered, and fVDI keeps only the one, so
 * there is nothing here to discriminate on; matching by name is the part
 * callers actually depend on.  Returning an ID for a name we hold beats
 * returning nothing because the format did not agree.
 */
/*
 * Case-insensitive compare of a NUL-terminated name against a fixed-width
 * field that may be blank-padded rather than terminated.  fVDI has no
 * case-insensitive compare of its own, and `equal` is neither.
 */
static int name_matches(const char *wanted, const char *field, long width)
{
    long i;
    ...
    for (i = 0; i < width; i++)
    {
        ...
        if (a == '\0')
        {
            /* Match if the field has nothing left but padding. */
...
long CDECL lib_vqt_name_and_id(Virtual *vwk, long format, const short *name_in,
                               long count, short *name_out)
{
    Fontheader *font;
    char wanted[64];
    long i;
    ...
    if (count < 0)
        count = 0;
    if (count > (long) sizeof(wanted) - 1)
        count = sizeof(wanted) - 1;
    for (i = 0; i < count; i++)
        wanted[i] = (char) name_in[i];
    wanted[count] = '\0';

    /* Trailing blanks are normal: names travel through fixed-width fields. */
    while (count > 0 && (wanted[count - 1] == ' ' || wanted[count - 1] == '\t'))
        wanted[--count] = '\0';

    font = vwk->real_address->writing.first_font;
    while (font)
    {
        if (name_matches(wanted, font->name, (long) sizeof(font->name)))
        {
            const unsigned char *p = (const unsigned char *) font->name;

            for (i = 0; i < (long) sizeof(font->name); i++)
                name_out[i] = p[i];
            return font->id;
        }
        font = font->next;
...
/*
 * width = lib_vst_setsize32(point, charw, charh, cellw, cellh)
 *
 * NVDI's VDI 252 with two intin words: the character width in 1/65536 of a
 * point, where plain vst_setsize takes whole points in one word.
 *
 * fVDI sizes fonts in whole points, so the fraction is rounded rather than
 * honoured, and the returned value is the size actually selected converted
 * back -- a caller that asks for 10.5pt and is told 10.0 can act on that,
 * whereas one told 10.5 when it got 10.0 lays text out wrongly and has no
 * way to know.
 */
long CDECL lib_vst_setsize32(Virtual *vwk, long point, short *charw, short *charh,
                             short *cellw, short *cellh)
{
    long points = (point + 0x8000L) >> 16;   /* 16.16 fixed -> whole points */

    if (points < 1)
        points = 1;

    return ((long) lib_vst_arbpt(vwk, points, charw, charh, cellw, cellh)) << 16;
}
